...
)
{
    ...
    while ((counter * RIOT_KEY_LENGTH) < bytesToDerive) {
        uint8_t out[RIOT_KEY_LENGTH];
        RIOT_KDF_SHA256(out, source, sourceSize, &counter,
                        fixed, fixedSize);
        memcpy(&result[cursor], out, MIN(bytesToDerive - cursor, sizeof(out)));
        cursor = counter;
    }
