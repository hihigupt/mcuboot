...
// encrypted keys. Expand the header buffer to ~128 bytes to support this.
{
    ...
    // Encode bytes
    Base64Encode(Context->Buffer, Context->Position, PEM, NULL, true);
    PEM += b64Len;
...
)
{
    ...
    if((strncmp((const char*)Pem, PEMhf[Type].header, PEMhf[Type].hLen)) ||
       (strncmp((const char*)&Pem[PemLen - PEMhf[Type].fLen], PEMhf[Type].footer, PEMhf[Type].fLen)))
    {
        return -1;
    }
    // Decode the data
    return Base64Decode((const char*)&Pem[PEMhf[Type].hLen], Der, DerLen);
}
