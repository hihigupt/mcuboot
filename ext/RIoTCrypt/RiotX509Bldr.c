...
)
{
    ...
    CHK(                DERStartEnvelopingOctetString(Tbs));
    CHK(                    DERStartSequenceOrSet(Tbs, true));
    CHK(                        DERAddSequenceOctets(Tbs, 0, TbsData->issuerKeyId, TbsData->issuerKeyIdLength));
    CHK(                    DERPopNesting(Tbs));
    CHK(                DERPopNesting(Tbs));
    CHK(            DERPopNesting(Tbs));
    if (TbsData->pathLength >= 0)
    {
        CHK(            DERStartSequenceOrSet(Tbs, true));
        CHK(                DERAddOID(Tbs, basicConstraintsOID));
        CHK(                DERAddBoolean(Tbs, true));
        ...
        {
            CHK(                    DERAddBoolean(Tbs, true));
            CHK(                    DERAddInteger(Tbs, TbsData->pathLength));
        }
        else
        {
            CHK(                    DERAddBoolean(Tbs, false));
...
//    CHK(                        DERPopNesting(Tbs));
    CHK(                        DERStartSequenceOrSet(Tbs, true));
    CHK(                            DERAddOID(Tbs, sha256OID));
    CHK(                            DERAddOctetString(Tbs, (uint8_t*)TbsData->derivationContext, TbsData->derivationContextLength));
    CHK(                        DERPopNesting(Tbs));
    CHK(                    DERPopNesting(Tbs));
    CHK(                DERPopNesting(Tbs));
    CHK(            DERPopNesting(Tbs));
...
{
    const uint8_t prefixDER[] = {0x30, 0x59, 0x30, 0x13, 0x06, 0x07, 0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x02, 0x01, 0x06, 0x08, 0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x03, 0x01, 0x07, 0x03, 0x42, 0x00};
    uint8_t derBuffer[DER_MAX_TBS] = { 0 };
    uint32_t derLen = DER_MAX_TBS;
    uint8_t* cursor = derBuffer;
    uint32_t cursorLen = DER_MAX_TBS;
    if(PEMtoDER((uint8_t*)pem, pemLen, R_PUBLICKEY_TYPE, derBuffer, &derLen) != 0)
    {
        return -1;
    }
    cursorLen = derLen;
    if(memcmp(cursor, prefixDER, sizeof(prefixDER)))
    {
        return -1;
    }
    cursor += sizeof(prefixDER);
    cursorLen -= sizeof(prefixDER);
    RIOT_STATUS riotErr = RIOT_SUCCESS;
    if((riotErr = RiotCrypt_ImportEccPub(cursor, cursorLen, pubKey)) != RIOT_SUCCESS)
    {
        return -1;
    }
    return 0;
...
int X509SetEccPrv(
    ...
    uint32_t               pemLen,
    RIOT_ECC_PUBLIC*       pubKey,
    RIOT_ECC_PRIVATE*      prvKey
)
{
    const uint8_t prefixDER1[] = {0x30, 0x77, 0x02, 0x01, 0x01, 0x04, 0x20};
    const uint8_t prefixDER2[] = {0xA0, 0x0A, 0x06, 0x08, 0x2A, 0x86, 0x48, 0xCE, 0x3D, 0x03, 0x01, 0x07, 0xA1, 0x44, 0x03, 0x42, 0x00};
    uint8_t derBuffer[DER_MAX_TBS] = { 0 };
    uint32_t derLen = DER_MAX_TBS;
    uint8_t* cursor = derBuffer;
    if(PEMtoDER((uint8_t*)pem, pemLen, R_ECC_PRIVATEKEY_TYPE, derBuffer, &derLen) != 0)
    {
        return -1;
    }
    if(memcmp(cursor, prefixDER1, sizeof(prefixDER1)))
    {
        return -1;
    }
    cursor += sizeof(prefixDER1);
    BigIntToBigVal(prvKey, cursor, RIOT_KEY_LENGTH);
    cursor += RIOT_KEY_LENGTH;
    if(memcmp(cursor, prefixDER2, sizeof(prefixDER2)))
    {
        return -1;
    }
    cursor += sizeof(prefixDER2);
    RIOT_STATUS riotErr = RIOT_SUCCESS;
    if((riotErr = RiotCrypt_ImportEccPub(cursor, derLen - (sizeof(prefixDER1) + RIOT_KEY_LENGTH + sizeof(prefixDER2)), pubKey)) != RIOT_SUCCESS)
    {
        return -1;
    }
    return 0;
...
//    uint8_t derBuffer[DER_MAX_TBS] = { 0 };
//    uint8_t encBuffer[((BIGLEN - 1) * sizeof(uint32_t))];
//    DERInitContext(&derCtx, derBuffer, DER_MAX_TBS);
//
//    CHK(DERStartSequenceOrSet(&derCtx, true));
//    CHK(    DERAddOID(&derCtx, ecdsaWithSHA256OID));
//    CHK(DERPopNesting(&derCtx));
//    CHK(DERStartEnvelopingBitString(&derCtx));
//    CHK(    DERStartSequenceOrSet(&derCtx, true));
//                BigValToBigInt(encBuffer, &Sig->r);
//    CHK(        DERAddIntegerFromArray(&derCtx, encBuffer, sizeof(encBuffer)));
