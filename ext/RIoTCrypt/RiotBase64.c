...
)
{
    ...
    else if (('\r' == Base64Character) || ('\n' == Base64Character) || (' ' == Base64Character))
    {
        *Val = 0;
        result = -2;
    }
...
)
{
    ...
    while ((result = base64toValue(EncodedString[length], &junkChar)) != -1)
    {
        if(result == -2) ignore++;
        length++;
    }
...
//    if ((base64CharacterCount(Input) % 4) != 0) {
//        return -1;
//    }
    ...
    while (charsRemaining >= 4)
    {
        ...
        while(base64toValue(Input[encodedIndex], &c1) == -2) encodedIndex++;
        while(base64toValue(Input[encodedIndex + 1], &c2) == -2) encodedIndex++;
        while(base64toValue(Input[encodedIndex + 2], &c3) == -2) encodedIndex++;
        while(base64toValue(Input[encodedIndex + 3], &c4) == -2) encodedIndex++;
